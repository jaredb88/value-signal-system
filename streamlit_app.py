...
with st.sidebar:
    ...
    # === Configuracion BTC (solo en esa seccion) ===
    elif seccion == "₿ Bitcoin (BTC)":
        st.divider()
        st.title("⚙️ Configuración")
        st.subheader("₿ Bitcoin")
        APORTE_BTC = st.number_input("Aporte BTC (USD/mes)", value=50, min_value=0, step=10, help="Bitcoin - tactico, ajustado por score")
...
if seccion == "🎯 Oportunidades":
    ...
    if _ops:
        ...
        for _o_op in sorted(_ops, key=lambda x: x["score"] or 0, reverse=True):
            ...
            with _c1_op:
                st.markdown(
                    f'<div style="font-size:2.0rem; font-weight:800; line-height:1.15; letter-spacing:-0.5px;">{_o_op["ticker"]}</div>'
    ...
    if _cl_sobre:
        ...
        for _a_op in _cl_sobre:
            ...
            with _c1_op:
                st.markdown(
                    f'<div style="font-size:2.0rem; font-weight:800; line-height:1.15; letter-spacing:-0.5px;">{_a_op.get("ticker", "?")}</div>'
...
if seccion == "🥇 Oro (GLD)":
    ...
    st.caption("Cobertura táctica: drawdown + momentum + macro · Update cada 30 min")

    # Cargar gld_data.json
    gld_json_path = _Path_gld(__file__).parent / "gld_data.json"
    if not gld_json_path.exists():
        st.error("⚠️ No se encontró gld_data.json. La tarea programada de GLD aún no ha generado datos.")
        st.stop()
    ...
    senales_orden = [
        ...
        ("dxy",          "💵 DXY (dólar)",       "dxy_actual",     "valor + percentil"),
        ("real_yield",   "📊 Real Yield TIPS",   "real_yield_pct", "% real yield"),
        ("vix",          "😱 VIX (miedo)",       "vix_actual",     "indice de volatilidad"),
    ]

    # Funcion auxiliar: genera explicacion + interpretación dinámica
    def _explicacion_senal(clave, score, detalle):
        """Devuelve (que_mide, interpretación_dinámica) segun el valor actual."""
        if clave == "drawdown":
            dd_pct = detalle.get("drawdown_pct", 0)
            max_52w = detalle.get("max_52w", 0)
            que = "Caída del precio actual respecto al maximo de las últimas 52 semanas. Score alto = precio deprimido = oportunidad de promediar."
            if abs(dd_pct) < 5:
                interp = f"GLD esta solo {abs(dd_pct):.1f}% por debajo de su maximo de ${max_52w:.2f}. Prácticamente en maximos. No hay oportunidad por correccion."
            elif abs(dd_pct) < 10:
                interp = f"GLD esta {abs(dd_pct):.1f}% por debajo de su maximo (${max_52w:.2f}). Corrección moderada, neutral."
            elif abs(dd_pct) < 20:
                interp = f"GLD esta {abs(dd_pct):.1f}% por debajo de su maximo (${max_52w:.2f}). Corrección significativa - momento atractivo para acumular."
            else:
                interp = f"GLD esta {abs(dd_pct):.1f}% por debajo de su maximo (${max_52w:.2f}). Caída fuerte - posible oportunidad o cambio de tendencia."
            return que, interp

        elif clave == "momentum":
            ret = detalle.get("ret_12_1_pct", 0)
            que = "Retorno entre hace 12 meses y hace 1 mes (factor Jegadeesh-Titman). Mide tendencia sostenida sin ruido del último mes. Score alto = trend alcista validado."
            if ret > 20:
                interp = f"GLD subio {ret:.1f}% en el año pasado (excluyendo ultimo mes). Tendencia alcista muy fuerte - confirma el bull market del oro."
            elif ret > 5:
                interp = f"GLD subio {ret:.1f}% en el año pasado. Tendencia alcista moderada."
            elif ret > -5:
                interp = f"GLD se movio {ret:+.1f}% en el año pasado. Prácticamente lateral."
            else:
                interp = f"GLD cayo {abs(ret):.1f}% en el año pasado. Tendencia bajista - evaluar si es punto de inflexión o caida estructural."
            return que, interp
            ...
            if pct < 25:
                interp = f"Ratio en percentil {pct:.0f}% (de los últimos 2 años). Oro muy barato relativo a equities - momento atractivo para rotar de acciones a oro."
            elif pct < 50:
            ...
            pct = detalle.get("percentil_2y", 50)
            que = "Índice del dólar vs canasta de divisas. Dolar fuerte historicamente reprime el oro (correlacion inversa). DXY alto = oro reprimido = oportunidad."
            if pct < 25:
                interp = f"DXY en {dxy:.2f}, percentil {pct:.0f}% (2y). Dolar debil - oro suele moverse bien con dólar debil, pero como ya es bajo, poca expectativa adicional."
            elif pct < 50:
                interp = f"DXY en {dxy:.2f}, percentil {pct:.0f}%. Dolar en zona media. Sin viento de cola fuerte para el oro."
            elif pct < 75:
                interp = f"DXY en {dxy:.2f}, percentil {pct:.0f}%. Dolar relativamente fuerte - oro reprimido. Si el dólar cae, el oro deberia rebotar."
            else:
                interp = f"DXY en {dxy:.2f}, percentil {pct:.0f}%. Dolar muy fuerte - oro estructuralmente reprimido. Cuando el ciclo del dólar termine, el oro deberia despegar."
            return que, interp
            ...
            que = "Índice de volatilidad del S&P 500. Mide el 'miedo' del mercado. VIX alto = panico = demanda de refugio = oro sube."
            if vix < 15:
    ...
    for clave, label, det_key, det_desc in senales_orden:
        ...
        if s is None:
            with st.container():
                ...
                with cl1:
                    ...
                    st.caption("⚠️ Señal no disponible")
                with cl2:
                    st.markdown("`— / 100`")
            continue
        ...
        with st.container():
            ...
            # Expander con explicacion + interpretación dinámica
            que_mide, interpretación = _explicacion_senal(clave, s, d)
            with st.expander("ℹ️ ¿Qué mide e interpretación"):
                st.markdown(f"**Qué mide:** {que_mide}")
                st.markdown(f"**📊 Tu lectura:** {interpretación}")
...
""")
            ...
            _orden_categorias_gld = ["precio", "etf_flows", "regulacion", "adopcion", "fed_macro", "sentimiento_onchain"]
...
if seccion == "₿ Bitcoin (BTC)":
    st.title("₿ Bitcoin (BTC) — Cripto Táctico")
    st.caption("Sistema cuantitativo de 6 señales: drawdown + sentimiento + ciclo halving + MVRV on-chain · Update cada 30 min")

    # Cargar btc_data.json
    btc_data_path = Path("btc_data.json")
    if not btc_data_path.exists():
        st.warning("No se encontró btc_data.json. La tarea programada aún no ha generado los datos.")
        st.stop()
    ...
    senales_orden_btc = [
        ("drawdown",      "📉 Drawdown vs ATH",      "drawdown_pct",       "Caida desde máximo histórico"),
    ...
    def _explicacion_senal_btc(clave, score, detalle):
        ...
        if clave == "drawdown":
            ...
            que = "Caída del precio actual respecto al máximo histórico (ATH). En BTC, drawdowns de -50% son normales en bear markets; -70% u -80% son zonas de panic-buy historicas."
            if abs(dd_pct) < 15:
                interp = f"BTC esta solo {abs(dd_pct):.1f}% por debajo de su ATH (${ath:,.0f}). Prácticamente en máximos - no hay oportunidad por correccion."
            elif abs(dd_pct) < 30:
                interp = f"BTC esta {abs(dd_pct):.1f}% por debajo de su ATH (${ath:,.0f}). Corrección menor en contexto cripto."
            elif abs(dd_pct) < 50:
                ...
                interp = f"BTC esta {abs(dd_pct):.1f}% por debajo de su ATH (${ath:,.0f}). Pánico extremo - zona de capitulación."
            return que, interp

        elif clave == "fear_greed":
            fg = detalle.get("fg_value", 50)
            classif = detalle.get("classification", "")
            que = "Índice Fear & Greed (0-100) calculado por alternative.me. Combina volatilidad, momentum, social media y dominance. Indicador contrario clásico: extreme fear = oportunidad."
            if fg < 20:
                interp = f"F&G en {fg}/100 ({classif}). EXTREME FEAR - señal contrarian clásica. Históricamente, los suelos de mercado se forman aqui."
            elif fg < 40:
            ...
            que = "Pi Cycle Top: ratio entre la media móvil de 111 días y 2x la media de 350 días. Cuando se cruzan (ratio cerca de 1.0) históricamente marca topes de ciclo (2013, 2017, 2021)."
            if ratio < 0.4:
                interp = f"Ratio en {ratio:.3f}. Lejísimos del tope - zona estructuralmente atractiva."
            elif ratio < 0.6:
                interp = f"Ratio en {ratio:.3f}. Lejos del tope - bear market o acumulación temprana."
            elif ratio < 0.8:
                interp = f"Ratio en {ratio:.3f}. Zona media - mercado normal."
            elif ratio < 0.95:
                interp = f"Ratio en {ratio:.3f}. Acercándose al tope - precaución."
            else:
            ...
            que = "Posición en el ciclo de 4 años del halving de Bitcoin. Patrón histórico: bull market ~12m post-halving, tope ~18m, bear market 18-30m, acumulación 30-48m."
            if meses < 6:
                interp = f"Mes {meses:.1f} post-halving (último: {last}). {fase} - acumulación temprana del bull cycle."
            elif meses < 12:
                interp = f"Mes {meses:.1f} post-halving (último: {last}). {fase} - momento clásico de subida fuerte."
            elif meses < 18:
                interp = f"Mes {meses:.1f} post-halving (último: {last}). {fase} - peligro de tope, reducir aporte."
            elif meses < 24:
                interp = f"Mes {meses:.1f} post-halving (último: {last}). {fase} - corrección inicial del bear market."
            elif meses < 30:
                interp = f"Mes {meses:.1f} post-halving (último: {last}). {fase} - mitad del bear market, oportunidad creciente."
            elif meses < 42:
                interp = f"Mes {meses:.1f} post-halving (último: {last}). {fase} - mejor zona histórica para acumular."
            else:
                interp = f"Mes {meses:.1f} post-halving (último: {last}). {fase} - acumulación final pre-halving."
            return que, interp

        elif clave == "momentum":
            ret = detalle.get("ret_12_1_pct", 0)
            que = "Retorno entre hace 12 meses y hace 1 mes (factor Jegadeesh-Titman). Mide trend sostenido sin ruido del último mes. En BTC, momentum extremo positivo (+100%) puede ser señal de tope."
            if ret > 100:
                interp = f"BTC subio {ret:.1f}% en el año - momento PARABÓLICO. Suele coincidir con topes de ciclo."
            elif ret > 30:
    ...
    for clave, label, det_key, det_desc in senales_orden_btc:
        ...
        if det_val is not None:
            ...
            elif det_key == "z_score":
                fecha = d.get("fecha", "—")
                det_txt = f"Z = {det_val:.3f} · {fecha}"
            else:
                det_txt = f"{det_val:.2f}"
        else:
            det_txt = "—"
    ...
    # ===== Noticias Macro de Bitcoin =====
    st.divider()
    st.header("📰 Noticias Macro de Bitcoin")
    st.caption("Cobertura de Fed, ETF spot (IBIT), regulacion, adopcion y on-chain - Actualizacion cada 2h")

    try:
        from pathlib import Path as _Path_news_gld
        import json as _json_news_gld
        _news_gld_path = _Path_news_gld(__file__).parent / "noticias_btc.json"
        if not _news_gld_path.exists():
            st.info("⏳ Las noticias de BTC aun no se han generado. Esperando la primera ejecucion de la tarea programada.")
        else:
            with open(_news_gld_path, "r", encoding="utf-8") as _f_news_gld:
                _news_gld_data = _json_news_gld.load(_f_news_gld)
...
""")
        ...
        st.warning(f"No se pudieron cargar las noticias de BTC: {_e_news_gld}")
